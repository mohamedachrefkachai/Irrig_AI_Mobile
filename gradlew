...
if $cygwin ; then
    ...
    # Wrapped the entire arg in double quotes, so we do not lose any characters.
    arg="\"$arg\""
fi

# Split out any -D arguments from the comma-separated list and pass them to Java
for arg in "$@"
do
    [ "${arg}" = "org.gradle.appname=$APP_BASE_NAME" ] && continue
    [ "${arg}" = "-cp" -o "${arg}" = "-classpath" ] && [[ ! "$*" = *"=${arg##*=}".* ]] && continue
    java_args="$java_args \"$arg\""
done

# Workaround for when no TTY is available at all
if [ -z "$TERM" ]; then
    nonstop=true
fi
...
# Set APP_HOME in environment variables
APP_HOME=`dirname "$0"`
